...
#' Transform an an ARMA(p, q) into a truncated infinite-order MA representation
#'
#' @description
...
#' Determines whether an ARMA(p,q) model is causal
#'
#' @description
...
#'
#' is_causal(model1) # TRUE
#' is_causal(model2) # FALSE
#'
#' @export
...
#' Computes the theoretical autocovariance of an ARMA(p,q) model
#'
#' @description
...
#' Computes the theoretical autocorrelation of an ARMA(p,q) model
#'
#' @description
...
get_theoretical_acf <- function(model, sigma, max_lag = 30) {
  if(!is_causal(model)) {
    print("AR part is not causal, so returned ACF may not be accurate.")
  }
...
#' Computes the sample autocovariance of an ARMA(p,q) model
#'
#' @description
